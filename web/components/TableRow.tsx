...
export default function TableRow({ last = false, controls = false, id = undefined, className, setUpdateTransactionRecord = undefined, ...props }: TableRowProps) {
      ...
      {controls == false ?
        ...
        <>
          ...
          {props.type != "header" ? <>
            ...
            <span className="w-[5%] inline-flex items-center justify-end pr-2.5">{props.type != "header" && "E"}</span>
            <span className="w-[5%] inline-flex items-center justify-center">{props.type != "header" && "D"}</span>
          </>
          }
        </>}
    </div >
